...
logger = logging.getLogger("focusedfuzz.focusedfuzz_fuzzer")
...
class Fuzzer(object):
    ...
    def __init__(self, focusedprogram, seed_files, workdir, target_opts=None,
                 input_placeholder='@@', afl_opts=None):
        self.focusedprogram = focusedprogram
        self.target_opts = target_opts
        self.workdir = workdir
        self.stat_file = os.path.join(self.workdir,
                                      focusedprogram.program_name, 'stat')
        self.stat = {}
        ...
        seeds = []
        logger.debug("Starting to classify the seeds for %s", focusedprogram.program_name)
        ...
        logger.debug("Classifying the seeds for %s ended", focusedprogram.program_name)

        use_qemu = False
        if focusedprogram.is_cgc():
            use_qemu = True

        logger.debug("fuzzing %s with opts: %s", self.focusedprogram.program_path,
                     str(target_opts))

        self._fuzzer = SFFuzzer(self.focusedprogram.program_path, self.workdir,
    ...
    def __str__(self):
        return "<Fuzzer:(tprogram:%s, workdir:%s)>" % \
            (self.focusedprogram, self.workdir)
    ...
    def __get_exec_result_on_input(self, input_file):
        ...
        executor = Executor(self.focusedprogram.program_path,
